...
- **Fill graph to previous Monday** - In contribution graph mode, pad the first week back to Monday so the graph stays square (default: on). Can be overridden per-tracker with `"fillToPreviousMonday"` in code blocks
...
| `fillToPreviousMonday` | boolean | true | In graph mode, pad the first week back to Monday for a square graph layout      |
